...
async function run() {
  try {
    ...
    await exec.exec('/bin/bash', ['-c', `"aws get-login-password --region ${core.getInput('region')} | docker login --username AWS --password-stdin ${core.getInput('account_id')}.dkr.ecr.${core.getInput('region')}.amazonaws.com"`])
